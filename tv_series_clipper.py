...
"""
电视剧短视频剪辑系统 - 专门针对电视剧连贯剧情剪辑
每集一个短视频，2-3分钟，保持跨集连贯性
"""
...
class TVSeriesClipper:
    def __init__(self, video_folder: str = "videos", output_folder: str = "series_clips"):
...
def process_all_episodes():
    ...
    clipper = TVSeriesClipper()
    
    # 获取所有字幕文件
    subtitle_files = []
    for file in os.listdir('.'):
        if file.endswith('.txt') and ('E' in file or 'S' in file or '集' in file):
    ...
    if not subtitle_files:
        print("❌ 未找到字幕文件")
        print("请将字幕文件放在项目根目录，文件名包含集数信息（如E01.txt, S01E01.txt）")
        return
